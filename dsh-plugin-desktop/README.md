...
The capture starts from `@deepseek-ai/dsh-subprocess`'s `scrubbedParentEnv()`, and captured names pass the same `SENSITIVE_ENV_PATTERN` and `DSH_ENV_PREFIX` checks before the fixed allowlist is applied. Credentials, `DSH_*` values, proxy and SSH-agent settings, and process startup hooks learned only from shell rc files are therefore not imported into Electron. DSH Desktop also removes an inherited `DEEPSEEK_API_KEY` before creating the launch-environment snapshot, so the key saved through Settings in `$DSH_HOME/.credentials.yaml` cannot be shadowed by a machine-wide or launcher-specific key. Other explicit launch variables remain unchanged. Ordinary DSH subprocesses apply the official scrub again; an explicit child environment may still deliberately add a value.
